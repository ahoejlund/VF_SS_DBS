function out = speech_segment_post(input)
...
extra_onsets = [   % add 8000 samples (? 200 ms which we pad with anyways)
    ];

extra_offsets = [  % subtract 8000 samples (? 200 ms which we pad with anyways) - and since we discard pauses of less than 500 ms in our MEG-analyses, it should be fine
    ];
...
SEGdir = fullfile(Pdir, subj, 'segments');
if ~exist(SEGdir,'dir')
    mkdir(SEGdir)
end
...
for c = input(2)
    for l = input(3)
        ...
        Sdir = fullfile(Pdir,subj,cond,sprintf('%s_post',letter));
        if ~exist(Sdir,'dir')
            mkdir(Sdir)
        end
        ...
        gaps_dur = floor([0.200*fs 0.050*fs 0.100*fs 0.100*fs 0.050*fs 0.150*fs]); % setting minimum gap duration to 50 ms
        ...
        if ~isempty(non_words)
            onsetidx_clean_old = onsetidx_clean;
            onsetidx_clean(non_words) = [];
            offsetidx_clean_old = offsetidx_clean;
            offsetidx_clean(non_words+1) = [];
        end
        
        if ~isempty(extra_onsets)
            onsetidx_clean = sortrows([onsetidx_clean; extra_onsets]);
        end
        
        if ~isempty(extra_offsets)
            offsetidx_clean = sortrows([offsetidx_clean; extra_offsets]);
        end
        ...
        hold on, plot(onsetidx(diff([onsetidx(1:end-1) offsetidx(2:end)],1,2)<speech_dur),ones(length(onsetidx(diff([onsetidx(1:end-1) offsetidx(2:end)],1,2)<speech_dur)),1),'ro','linewidth',1)
        hold on, plot(onsetidx_clean_old(non_words),ones(length(non_words),1),'mo','linewidth',1)
        ...
        save(fullfile(Sdir,sprintf('corrections_%s_%s_%s.mat',subj,cond,letter)),'non_words','move_onsets','move_offsets','extra_onsets','extra_offsets');
        
        if onsetidx_clean(1) ~= 0
            segments = [0 onsetidx_clean'; offsetidx_clean' 60.6*fs]./fs;
        end            
        save(fullfile(SEGdir,sprintf('%s_%s.mat',conds{cond_seq(c)},letter)),'segments');
